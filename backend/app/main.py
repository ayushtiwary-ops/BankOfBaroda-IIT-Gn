...
Endpoints (every one is authenticated + scoped):
...
Trust boundaries enforced here:
  * step-up arrives ONLY as a signed verifier assertion (request body),
    validated server-side. A self-asserted ``?verified=true`` cannot move trust.
  * clients get a generic decision; the rich SocAssessment (reasons,
    trust score, feature contributions) goes ONLY to the audit/SOC plane.
  * CORS is an explicit allowlist, never "*"; secrets come from config.
"""
import hashlib
import hmac
import json
...
# Startup config - fails LOUD in prod if a required secret is unset.
settings = Settings.from_env()
# Engine load - fails LOUD in prod if the real model artifact is missing.
engine = TrustEngine(settings=settings)
# Keyed audit chain (secret + audit integrity).
...
# keyed PII vault - per-identity material, crypto-shreddable on erasure.
pii_vault = KeyedPiiStore()
authenticate, require = make_auth(settings)

DEMO_DIR = Path(__file__).resolve().parents[2] / "demo"

# in-process caches (documented to move to Redis in prod alongside).
# SECURITY: bounded LRU; key = (caller, idempotency_key) → (fingerprint, response).
_IDEM_MAX = 10_000
_idem_cache: "OrderedDict[tuple[str, str], tuple[str, RiskAssessment]]" = OrderedDict()
# SECURITY: accepted step-ups (anti-fatigue) tracked SEPARATELY from
...
# SECURITY: explicit allowlist from config, never "*".
app.add_middleware(
    CORSMiddleware,
...
def health():
    return {
        ...
        "service": "pramaan",
        "mode": settings.mode,
        "model": engine.model_mode,          # provenance honesty stamp
...
def ingest_event(event: IdentityEvent,
    ...
    # SECURITY: idempotency is bound to (caller, key, PAYLOAD fingerprint).
    ...
    # stash per-identity material in the keyed vault under a non-reversible
    # ref; the audit chain carries only the token + ref, never plaintext.
    # a tombstoned (erased) identity is scored but NOT re-vaulted - erasure
    ...
    audit.append(payload)                     # FULL reasons → SOC plane
...
def step_up_outcome(identity_id: str, body: StepUpAssertionRequest,
    ...
    # SECURITY: the outcome must be a SIGNED verifier assertion.
    ...
    # SECURITY: anti-fatigue cap applies ONLY to VALIDATED step-ups, so
...
def identity_state(identity_id: str, ctx=Depends(require("identity:read"))):
    # SECURITY: formerly public IDOR/enumeration endpoint, now
...
def erase_identity(identity_id: str, ctx=Depends(require("identity:erase"))):
    # SECURITY: crypto-shredding. Destroy the per-identity key so the
...
def audit_verify(ctx=Depends(require("audit:read"))):
    # head_checkpoint  lets the SOC detect tail-truncation out-of-band.
