...
- All **141 ATO positives** are retained in `data/samples/rba_sample.csv` (+ a seeded, order-preserved
  ~15k negative sample, kept small for GitHub) for the serving-model export and CI smoke. The full-set
  headline numbers are regenerated by `src/train_rba_full.py`, which streams the full `rba-dataset.csv`
  out of the verified zip in chunks and scores the entire 6.79M-login held-out test split (see
  `docs/EVALUATION.md`). The small sample alone does not reproduce the full-data figures.
